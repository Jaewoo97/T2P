...
from .layers_v1 import DecoderLayer,  TBIFormerBlock
...
class TBIFormer(nn.Module):
    ...
    def __init__(
        ...
        self.proj_inverse=nn.Linear(d_model, (opt.num_joints-1)*3)
        ...
        num_heads, hivt_dropout, num_temporal_layers, local_radius, parallel, num_modes = 8, 0.1, 4, None, False, opt.num_modes
    ...
    def forward_original(self, src, n_person, trj_dist):
        ...
        dec_output = self.l1(dec_output)
        dec_output = self.l2(dec_output)
        dec_output = dec_output.view(bn, self.output_time, -1)
        dec_out = self.proj_inverse(dec_output)
        return dec_out
