# Pi network is modified
# It takes same input with Loc network
...
class GRUDecoder_hivt_2(nn.Module):
    ...
    def __init__(self,
        ...
        super(GRUDecoder_hivt_2, self).__init__()
...
class MLPDecoder_hivt_2(nn.Module):
    ...
    def __init__(self,
                 ...
                 uncertain: bool = True,
                 min_scale: float = 1e-3) -> None:
        super(MLPDecoder_hivt_2, self).__init__()
        self.input_size = global_channels
        self.hidden_size = local_channels
        self.future_steps = future_steps
        self.num_modes = num_modes
        ...
        self.aggr_embed = nn.Sequential(
            nn.Linear(self.input_size + self.hidden_size, self.hidden_size*2),
        ...
        self.loc = nn.Sequential(
            ...
            nn.LayerNorm(self.hidden_size*2),
            nn.ReLU(),
            nn.Linear(self.hidden_size*2, self.future_steps * 3))
        if uncertain:
            self.scale = nn.Sequential(
                nn.Linear(self.hidden_size*2, self.hidden_size*2),
                nn.LayerNorm(self.hidden_size*2),
                ...
                nn.Linear(self.hidden_size*2, self.future_steps * 3))
        self.pi = nn.Sequential(
            nn.Linear(self.hidden_size + self.input_size, self.hidden_size*2),
            nn.LayerNorm(self.hidden_size*2),
            nn.ReLU(),
            nn.Linear(self.hidden_size*2, self.hidden_size),
            nn.LayerNorm(self.hidden_size),
    ...
    def forward(self,
        ...
        out_embed = self.out_embed(out)
        if self.uncertain:
            scale = F.elu_(self.scale(out), alpha=1.0).view(self.num_modes, -1, self.future_steps, 3) + 1.0
            scale = scale + self.min_scale  # [F, N, H, 3]
            return torch.cat((loc, scale), dim=-1), pi, out_embed  # [F, N, H, 4], [N, F]
        else:
            return loc, pi, out_embed  # [F, N, H, 3], [N, F]
        
class MLPDecoder_hivt_2_1(nn.Module):
    ...
    def __init__(self,
                 ...
                 uncertain: bool = True,
                 min_scale: float = 1e-3) -> None:
        super(MLPDecoder_hivt_2_1, self).__init__()
        self.input_size = global_channels
        self.hidden_size = local_channels
        self.future_steps = future_steps
        self.num_modes = num_modes
