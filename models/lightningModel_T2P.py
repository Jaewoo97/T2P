...
from utils_.soft_dtw_cuda import SoftDTW
...
class TBIFormer(nn.Module):
    ...
    def forward(self, batch_data, mode):
        ...
        trj_dist = bulding_TRPE_matrix(input_seq.reshape(B,N,-1,self.num_joints,3), self.opt)  #  trajectory similarity distance
        ...
        src = dct.dct(offset)
        enc_feat = self.forward_encode_body(src, N,  trj_dist)
        ...
        rec_ = self.forward_local(src, N,  trj_dist, traj_feats, enc_feat)
        rec = dct.idct(rec_)
    ...
    def forward_encode_body(self, src, n_person, trj_dist):
        '''
        src_seq:  B*N, T, J*3
        '''
        bn = src.shape[0]
    ...
    def forward_local(self, src, n_person, trj_dist, traj_query, enc_out):
        '''
        src_seq:  B*N, T, J*3
        '''
