...
class GlobalInteractor_hivt(nn.Module):
    ...
    def __init__(self,
        ...
        if rotate:
            self.rel_embed = MultipleInputEmbedding_hivt(in_channels=[edge_dim, 2], out_channel=embed_dim)
        else:
            self.rel_embed = SingleInputEmbedding_hivt(in_channel=edge_dim, out_channel=embed_dim)
        self.global_interactor_layers = nn.ModuleList(
            [GlobalInteractorLayer_hivt(embed_dim=embed_dim, num_heads=num_heads, dropout=dropout)
    ...
    def forward(self,
        ...
        if data['rotate_mat'] is None:
            ...
            # rel_pos = rel_pos.unsqueeze(-2)
            # rel_pos[:,:,:2] = torch.bmm(rel_pos[:,:,:2], data['rotate_mat'][edge_index[1]])
...
class GlobalInteractorLayer_hivt(MessagePassing):
    ...
    def __init__(self,
                 ...
                 dropout: float = 0.1,
                 **kwargs) -> None:
        super(GlobalInteractorLayer_hivt, self).__init__(aggr='add', node_dim=0, **kwargs)
        self.embed_dim = embed_dim
