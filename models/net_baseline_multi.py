...
from .layers_multi import DecoderLayer, TBIFormerBlock
...
class Decoder(nn.Module):
    ...
    def __init__(
            self,  n_layers, n_head, d_k, d_v,
            d_model, d_inner,  dropout=0.1, num_modes=1, device='cuda'):
        super().__init__()
        self.layer_stack = nn.ModuleList([
            DecoderLayer(d_model, d_inner, n_head, d_k, d_v, num_modes, dropout=dropout)
            for _ in range(n_layers)])
        self.num_modes = num_modes
        self.mode_proj = nn.Linear(d_model, num_modes * d_model)
    ...
    def forward(self, trg_seq, enc_output, return_attns=False):
        ...
        dec_enc_attn_list = []
        trg_seq = trg_seq.unsqueeze(0).repeat(self.num_modes, 1, 1, 1)
        B, N, D = enc_output.size()
        enc_output = self.mode_proj(enc_output).view(B, N, D, self.num_modes).permute(3,0,1,2)
...
class TBIFormer(nn.Module):
    ...
    def __init__(
        ...
        self.decoder = Decoder(d_model=d_model, d_inner=d_inner,
                               n_layers=n_layers, n_head=n_head, d_k=d_k, d_v=d_v, dropout=dropout, num_modes=opt.num_modes, device=self.device)
        ...
        self.mlp = nn.Sequential(nn.Conv1d(in_channels=opt.num_joints*3, out_channels=d_model, kernel_size=kernel_size1,
                                             bias=False),
                                   nn.ReLU(inplace=False),
        ...
        self.proj_inverse=nn.Linear(d_model, opt.num_joints*3)
        # self.l1=nn.Linear(d_model+d_traj_query, d_model*8)
        self.l1=nn.Linear(d_model, d_model*8)
        self.l2=nn.Linear(d_model*8, d_model*self.output_time)
    ...
    def forward(self, src, n_person, trj_dist, traj_query):
        '''
        src_seq:  B*N, T, J*3
        '''
        ...
        if self.dataset == 'mocap_umpm': index = [[9, 10, 11], [12, 13, 14], [1, 2, 3], [4, 5, 6], [0, 7, 8]]  # 5 body parts
        # if self.dataset == 'mocap_umpm': index = [[8, 9, 10], [11, 12, 13], [1, 2, 3], [4, 5, 6], [0, 7, 14]]  # 5 body parts (original code)
    ...
    def forward_original(self, src, n_person, trj_dist):
        ...
        dec_output = dec_output.reshape(self.num_modes, bn, 1, -1)

        # =======  FC ============
        dec_output = self.l1(dec_output)
        dec_output = self.l2(dec_output)
        dec_output = dec_output.view(self.num_modes, bn, self.output_time, -1)
        dec_out = self.proj_inverse(dec_output)
        return dec_out
