# v2: motion embedding concat 해서 같이 처리
...
class AAEncoder_hivt(MessagePassing):
    ...
    def forward(self,
        ...
        if self.parallel:
            ...
            if rotate_mat is None:
                ...
                # x = x.unsqueeze(-2)
                # x[:,:,:2] = torch.bmm(x[:,:,:2], rotate_mat)
                # center_embed = self.center_embed(x.squeeze(-2))
    ...
    def message(self,
        ...
        if rotate_mat is None:
            ...
            #     x_j = x_j.squeeze()
            # x_j = x_j.unsqueeze(-2)
            # x_j[:,:,:2] = torch.bmm(x_j[:,:,:2], center_rotate_mat)
            # edge_attr = edge_attr.unsqueeze(-2)
            # edge_attr[:,:,:2] = torch.bmm(edge_attr[:,:,:2], center_rotate_mat)
            # nbr_embed = self.nbr_embed([x_j.squeeze(-2), edge_attr.squeeze(-2)])
