...
def perspective_projection(points, translation=None,rotation=None, 
                           focal_length=480, camera_center=None, img_size=512, normalize=True):
    """
...
def body_mesh_projection2image(j3d_preds, cam_preds, vertices=None, input2org_offsets=None):
    ...
    pj2d = perspective_projection(j3d_preds,translation=pred_cam_t,focal_length=480, normalize=True)
    projected_outputs = {'cam_trans':pred_cam_t, 'pj2d': pj2d.float()}
    if vertices is not None:
        projected_outputs['verts_camed'] = perspective_projection(vertices.clone().detach(),translation=pred_cam_t,focal_length=480, normalize=True)
...
class SMPLA_parser(nn.Module):
    ...
    def forward(self, betas=None, thetas=None, root_align=True):
        ...
        if baby_mask.sum()>0:
            ...
            verts, joints = torch.zeros(person_num, 6890, 3, device='cuda').float(), torch.zeros(person_num, 54+17, 3, device='cuda').float()
