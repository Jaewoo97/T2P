...
PARSED_VERSION = 'jrdb_bev_v3_input_2_output_5_frameRep_3'
BASE_PARSED_DIR = '/ssd4tb/jaewoo/t2p/parsed_jrdb/'
RAW_FILE_NAMES_JRDB = {'train': BASE_PARSED_DIR+PARSED_VERSION,
                       'val': BASE_PARSED_DIR+PARSED_VERSION,
                       'test': BASE_PARSED_DIR+PARSED_VERSION}
...
class jrdb(Dataset):
    def __init__(self,
        ...
        self.train_val_list = np.load(os.path.join(BASE_PARSED_DIR, PARSED_VERSION+'_trainVal_split_names.npy'), allow_pickle=True).item()
        if split == 'train': self._raw_file_names = [BASE_PARSED_DIR + PARSED_VERSION + '/' + item_name for item_name in self.train_val_list['train']]
        else: self._raw_file_names = [BASE_PARSED_DIR + PARSED_VERSION + '/' + item_name for item_name in self.train_val_list['val']]
    ...
    def process_jrdb(self, tokens: str,
        ...
        for token in tqdm(tokens):  # token: N_sequences, N_agents, T, 45 에서 N_sequences 의 index
            ...
            N, T, N_JOINTS, XYZ = raw_file['data'].shape
            
            seq_data_all = torch.zeros((MAX_NUM_PERSON, T, N_JOINTS, XYZ))
            
            seq_data = raw_file['data'].float()
            seq_data_all[:N, ...] = seq_data
            body_xyz = seq_data_all.clone()
            ...
            input_traj, output_traj = seq_data_all[:, :self.ref_time, 0], seq_data_all[:, self.ref_time:, 0]     # Only hip joint info
            edge_index = torch.LongTensor(list(permutations(range(MAX_NUM_PERSON), 2))).t().contiguous()
            ...
            x[:,0] = torch.zeros(MAX_NUM_PERSON, 3)
            y = x[:,self.ref_time:]
            
            padding_mask_all = torch.ones((MAX_NUM_PERSON, T)).bool()
            padding_mask = raw_file['padding']
            padding_mask_all[:N, ...] = padding_mask
            
            bos_mask = torch.zeros(MAX_NUM_PERSON, self.ref_time, dtype=torch.bool).cuda()
            bos_mask[:, 0] = ~padding_mask_all[:, 0]
            bos_mask[:, 1: self.ref_time] = padding_mask_all[:, : self.ref_time-1] & ~padding_mask_all[:, 1: self.ref_time]
            
            rotate_angles = torch.zeros(MAX_NUM_PERSON, dtype=torch.float)

            for actor_id in range(MAX_NUM_PERSON):
            ...
            if self.rotate:
                rotate_mat = torch.empty(MAX_NUM_PERSON, 3, 3)
            ...
            processed = {
                ...
                'padding_mask': padding_mask_all,
                'edge_index': edge_index,
                'bos_mask': bos_mask,
                'y': y,
                'num_nodes': MAX_NUM_PERSON,
...
if __name__ == '__main__':
    ...
    spec_args = {'dataset': 'jrdb', 'n_jobs': 4, 't_h': 2, 't_f': 6, 'res': 2, 'ref_time':30, 'lseg_len': 10, 'lseg_angle_thres': 30, 'lseg_dist_thres': 2.5, 'random_flip': True, 'rotate': True}
    # A1D = mocap_UMPM('val', process_dir='preprocessed/mocap_UMPM_input25', root='data/Mocap_UMPM',
    #                          process=True, spec_args=spec_args)
    A1D = jrdb('val', process_dir='/ssd4tb/jaewoo/t2p/t2p/preprocessed/'+PARSED_VERSION, root='/ssd4tb/jaewoo/t2p/parsed_jrdb/jrdb_bev_v3',
                             process=True, spec_args=spec_args)
