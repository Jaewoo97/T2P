...
RAW_FILE_NAMES_3DPW = {'train': '/ssd4tb/jaewoo/t2p/t2p/data/poseData.pkl', 'val': '/ssd4tb/jaewoo/t2p/t2p/data/poseData.pkl', 'test': '/ssd4tb/jaewoo/t2p/t2p/data/poseData.pkl'}
def Rx(theta):
...
class _3DPW(Dataset):
    ...
    def __init__(self,
        ...
        # self._raw_file = np.load(RAW_FILE_NAMES_3DPW[split])
        B, N, T, JXYZ = self._raw_file.shape
        # self._raw_file = np.array(np.dot(self._raw_file.reshape(-1, 3), R)).reshape((B, N, T, JXYZ))
        self._raw_file = self._raw_file.reshape((B, N, T, JXYZ//3, 3))[..., [2,0,1]]
    ...
    def process(self) -> None:
        ...
        if self.n_jobs > 1:
            ...
            procs = []
            for proc_id in range(self.n_jobs):
                process = Process(target=self.process_3dpw, args=(raw_file_list[proc_id],self._local_radius))
                process.daemon = True
                process.start()
    ...
    def process_3dpw(self, tokens: str,
                         radius: float) -> Dict:

        for token in tqdm(tokens):  # token: N_sequences, N_agents, T, 39 에서 N_sequences 의 index
            N, T, _, _ = self._raw_file[token].shape
            # seq_data = self._raw_file[token].reshape(N,T,13,3)
...
if __name__ == '__main__':
    ...
    spec_args = {'dataset': '3dpw', 'n_jobs': 0, 't_h': 2, 't_f': 6, 'res': 2, 'ref_time':10, 'lseg_len': 10, 'lseg_angle_thres': 30, 'lseg_dist_thres': 2.5, 'random_flip': True, 'rotate': True}
    # A1D = mocap_UMPM('val', process_dir='preprocessed/mocap_UMPM_input25', root='data/Mocap_UMPM',
    #                          process=True, spec_args=spec_args)
    A1D = _3DPW('val', process_dir='/ssd4tb/jaewoo/t2p/t2p/preprocessed/3dpw_input10_v2', root='data/',
                             process=True, spec_args=spec_args)
