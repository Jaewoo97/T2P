...
PED_CLASS = {}
VEH_CLASS = {}
SPLIT_NAME = {'mocap_UMPM': {'train': 'train', 'val': 'val', 'test': 'val'}}
RAW_FILE_NAMES_UMPM = {'train': '/ssd4tb/jaewoo/t2p/t2p/data/Mocap_UMPM/train_3_75_mocap_umpm.npy', 'val': '/ssd4tb/jaewoo/t2p/t2p/data/Mocap_UMPM/test_3_75_mocap_umpm.npy', 'test': '/ssd4tb/jaewoo/t2p/t2p/data/Mocap_UMPM/test_3_75_mocap_umpm.npy'}
...
class mocap_UMPM(Dataset):
    ...
    def __init__(self,
        ...
        self._raw_file = np.load(RAW_FILE_NAMES_UMPM[split])
        self._raw_file_names = [i for i in range(self._raw_file.shape[0])]
            
        self._processed_file_names = [str(f) + '.pt' for f in range(len(self._raw_file))]
        self._processed_paths = [os.path.join(self.processed_dir, f) for f in self._processed_file_names]
        super(mocap_UMPM, self).__init__(root, transform=transform)
    ...
    def process(self) -> None:
        ...
        if self.n_jobs > 1:
            ...
            procs = []
            for proc_id in range(self.n_jobs):
                process = Process(target=self.process_mocap_UMPM, args=(raw_file_list[proc_id],self._local_radius))
                process.daemon = True
                process.start()
            ...
            self.process_mocap_UMPM(self._raw_file_names, self._local_radius)
    ...
    def process_mocap_UMPM(self, tokens: str,
                         radius: float) -> Dict:

        for token in tqdm(tokens):  # token: N_sequences, N_agents, T, 45 에서 N_sequences 의 index
            N, T, _ = self._raw_file[token].shape
            seq_data = self._raw_file[token].reshape(N,T,15,3)
            new_order = [2,0,1]
            seq_data = seq_data[...,new_order]
...
class mupots(Dataset):
    ...
    def __init__(self,
        ...
        self._processed_file_names = [str(f) + '.pt' for f in range(len(self._raw_file))]
        self._processed_paths = [os.path.join(self.processed_dir, f) for f in self._processed_file_names]
        super(mupots, self).__init__(root, transform=transform)
    ...
    def process(self) -> None:
        ...
        if self.n_jobs > 1:
            ...
            procs = []
            for proc_id in range(self.n_jobs):
                process = Process(target=self.process_mupots, args=(raw_file_list[proc_id],self._local_radius))
                process.daemon = True
                process.start()
                procs.append(process)
            ...
            self.process_mupots(self._raw_file_names, self._local_radius)
    ...
    def process_mupots(self, tokens: str,
                         radius: float) -> Dict:
...
if __name__ == '__main__':
    ...
    spec_args = {'dataset': 'mocap_UMPM', 'n_jobs': 0, 't_h': 2, 't_f': 6, 'res': 2, 'ref_time':25, 'lseg_len': 10, 'lseg_angle_thres': 30, 'lseg_dist_thres': 2.5, 'random_flip': True, 'rotate': True}
    A1D = mocap_UMPM('train', process_dir='preprocessed/mocap_UMPM_input25_v2', root='data/Mocap_UMPM',
                             process=True, spec_args=spec_args)
    # A1D = mocap_UMPM('val', process_dir='preprocessed/test', root='data/Mocap_UMPM',
    #                          process=True, spec_args=spec_args)
